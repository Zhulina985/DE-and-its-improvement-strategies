"""
Crested Porcupine Optimizer with DE-NPS Strategies
Integrates DE-NPS strategies (bi-stage parameter adaptation, logarithmic spiral perturbation,
and diversity enhancement) into CPO algorithm.
"""

import numpy as np
from src.algorithms.cpo_algorithm import CrestedPorcupineOptimizer
from .denps_adapter import DENPSAdapter


class CPO_NPS(CrestedPorcupineOptimizer):
    def __init__(self, func, bounds, pop_size=50, max_generations=1000):
        super().__init__(func, bounds, pop_size, max_generations)
        
        # Initialize DE-NPS adapter
        self.denps = DENPSAdapter(self.dim, self.pop_size, self.max_generations, self.bounds)
    ...
    def optimize(self):
        ...
        self.denps.increment_nfes(self.pop_size)
        ...
        for t in range(1, self.max_generations + 1):
            ...
            if target_size < current_pop_size:
                ...
                # Update adapter population size
                self.denps.pop_size = current_pop_size
                self.denps.count = self.denps.count[:current_pop_size]
            ...
            # Get adaptive parameter from DE-NPS (for exploration-exploitation balance)
            adaptive_param = self.denps.get_adaptive_parameter()
            ...
            for i in range(current_pop_size):
                ...
                # Use adaptive parameter to influence exploration/exploitation decision
                ...
                # Apply logarithmic spiral perturbation with probability
                if np.random.rand() < 0.3:  # 30% chance to apply perturbation
                    new_pos = self.denps.logarithmic_spiral_perturbation(X_i, new_pos)
            ...
            # Apply diversity enhancement mechanism
            population, fitness = self.denps.update_stagnant_individuals(
                population, fitness, best_solution, best_fitness
            )
            
            # Re-evaluate updated individuals
            for i in range(current_pop_size):
                if self.denps.count[i] == 0 and i < len(population):
                    # Individual was updated, re-evaluate
